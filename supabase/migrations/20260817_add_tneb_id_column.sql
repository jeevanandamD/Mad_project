-- =============================================================================
-- ADD TNEB ID COLUMN (Missing column - must run first)
-- =============================================================================
-- This migration adds the tneb_id column to profiles table
-- It must be run before any other migrations that reference tneb_id

alter table public.profiles add column if not exists tneb_id text default '';

-- Add check constraint for TNEB ID format if not exists
do $$
begin
    if not exists (
...
-- =============================================================================
-- ROLE TRIGGER - allow worker self-registration, never 'admin'
-- =============================================================================
